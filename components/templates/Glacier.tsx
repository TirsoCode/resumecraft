...
export default function GlacierTemplate({ data }: Props) {
  ...
  return (
    <div style={{ fontFamily: "var(--font-instrument), system-ui", color: "#1A1918", background: "#F0F9FF", padding: p4048 }}>
      ...
      {data.settings.sections.experience && experience.length > 0 && <section style={{ marginBottom: p26 }}>
        ...
        {experience.map((item) => (
          <div key={item.id} style={{ marginBottom: p18 }}>
            <div style={{ display: "flex", justifyContent: "space-between" }}><h3 style={{ fontSize: 13*s, fontWeight: 700, margin: 0 }}>{item.position}</h3><span style={{ fontSize: 10*s, fontFamily: "var(--font-jetbrains), monospace", color: "#0369A1" }}>{item.startDate} — {item.endDate}</span></div>
      ...
      {data.settings.sections.education && education.length > 0 && <section style={{ marginBottom: p26 }}>
        ...
        {education.map((item) => <div key={item.id} style={{ marginBottom: p10 }}><div style={{ display: "flex", justifyContent: "space-between" }}><h3 style={{ fontSize: 12*s, fontWeight: 700, margin: 0 }}>{item.degree}</h3><span style={{ fontSize: 10*s, fontFamily: "var(--font-jetbrains), monospace", color: "#0369A1" }}>{item.startDate} — {item.endDate}</span></div><p style={{ fontSize: 11*s, color: "#0369A1", margin: `${2*s}px 0 0` }}>{item.institution}</p></div>)}
      </section>}
