...
export default function IvoryTemplate({ data }: Props) {
  ...
  return (
    <div style={{ fontFamily: "'Playfair Display', Georgia, serif", color: "#44403C", background: "#FDFDFA", padding: p4452 }}>
      ...
      {data.settings.sections.experience && experience.length > 0 && <section style={{ marginBottom: p28 }}>
        ...
        {experience.map((item) => <div key={item.id} style={{ marginBottom: p18 }}>
          <div style={{ display: "flex", justifyContent: "space-between", borderBottom: `1px dashed #D6D3D1`, paddingBottom: p4, marginBottom: p6 }}><h3 style={{ fontSize: 14*s, fontWeight: 600, margin: 0 }}>{item.position}</h3><span style={{ fontSize: 10*s, fontFamily: "'JetBrains Mono', monospace", color: "#78716C" }}>{item.startDate} — {item.endDate}</span></div>
