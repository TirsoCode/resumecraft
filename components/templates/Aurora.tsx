...
export default function AuroraTemplate({ data }: Props) {
  ...
  return (
    <div style={{ fontFamily: "'Instrument Sans', system-ui", color: "#1A1918", background: "#ECFDF5", padding: p3644 }}>
      ...
      {data.settings.sections.summary && summary && <section style={{ marginBottom: p24 }}><p style={{ fontSize: 12*s, lineHeight: 1.7, color: "#065F46", margin: 0 }}>{summary}</p></section>}
      ...
      {data.settings.sections.references && references.length > 0 && <section style={{ marginBottom: p24 }}>
        ...
        {references.map((item) => <div key={item.id} style={{ marginBottom: p10 }}><h3 style={{ fontSize: 12*s, fontWeight: 700, margin: 0 }}>{item.name}</h3><p style={{ fontSize: 11*s, color: "#059669", margin: p20_0_4 }}>{item.company}{item.relationship && ` • ${item.relationship}`}</p><p style={{ fontSize: 10*s, color: "#059669", margin: p20_0_0 }}>{item.email}{item.phone && ` • ${item.phone}`}</p></div>)}
      </section>}
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: p0_32 }}>
        ...
        <div>
          {data.settings.sections.languages && languages.length > 0 && <section style={{ marginBottom: p20_px }}><h2 style={{ fontSize: 10*s, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.14em", color: accentColor, margin: `0 0 ${p12}` }}>Idiomas</h2>{languages.map((l) => <div key={l.id} style={{ fontSize: 11*s, marginBottom: p4 }}><span style={{ fontWeight: 700 }}>{l.language}</span> <span style={{ color: "#059669" }}>{l.level}</span></div>)}</section>}
