        ...
        <aside style={{ background: "#1A1918", color: "#F3F2EE", padding: "48px 28px", minHeight: "100%" }}>
          ...
          <div style={{ marginBottom: 28 }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
              {personal.email && <span style={{ fontSize: 10, color: "#E4E2DC", fontFamily: "var(--font-instrument), sans-serif" }}>{personal.email}</span>}
            ...
            <div style={{ marginBottom: 28 }}>
              ...
              {skills.map((cat) => (
                <div key={cat.id} style={{ marginBottom: 10 }}>
                  <p style={{ fontSize: 9, fontWeight: 600, color: "#9C9890", margin: "0 0 4px", fontFamily: "var(--font-instrument), sans-serif", textTransform: "uppercase", letterSpacing: "0.08em" }}>
                    {cat.category}
                  </p>
            ...
            <div style={{ marginBottom: 28 }}>
              ...
              {languages.map((l) => (
                <div key={l.id} style={{ marginBottom: 6 }}>
                  <span style={{ fontSize: 10, color: "#E4E2DC", fontFamily: "var(--font-instrument), sans-serif", fontWeight: 500 }}>{l.language}</span>
                  <span style={{ fontSize: 10, color: "#6B6860", marginLeft: 6, fontFamily: "var(--font-instrument), sans-serif" }}>{l.level}</span>
                </div>
              ))}
            </div>
          )}
              ...
              {experience.map((item) => (
                <div key={item.id} style={{ marginBottom: 24 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 2 }}>
                    ...
                    <span style={{ fontSize: 10, color: "#9C9890", fontFamily: "var(--font-jetbrains), monospace", marginLeft: 12 }}>
                      {item.startDate} — {item.endDate}
                    </span>
                  </div>
                  <p style={{ fontSize: 11, color: accentColor, margin: "0 0 8px", fontFamily: "var(--font-instrument), sans-serif", fontWeight: 500 }}>
                    {item.company}
                  </p>
